...
from .serializers import (
    login_fields_serializer, register_fields_serializer,register_admin_fields_serializer,
...
register_admin_serializer = auth_namespace.model('Register admin serializer', register_admin_fields_serializer)
...
class AdminCreationView(Resource):

    @auth_namespace.expect(register_admin_serializer)
    @auth_namespace.doc(
        description="""
    ...
    def post(self):
        ...
        admin = Admin.query.filter_by(school_mail=data.get('school_email', None)).first()
        
        if admin:
            return {'message' : 'School email already exists'} , HTTPStatus.CONFLICT
        # Create new user
        identifier=random_char(10)
        new_user = Admin(
